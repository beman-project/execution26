...
namespace
{
    ...
    struct test_sender
    {
        ...
        struct upstream
        {
            ...
            auto set_error(auto&& error) && noexcept -> void 
            {
                if constexpr (std::same_as<Tag, test_std::set_error_t> && 1u == std::tuple_size_v<Result>)
                    if constexpr (
                        std::same_as<std::decay_t<std::tuple_element_t<0, Result>>, std::decay_t<decltype(error)>>
                    )
        ...
        struct state
        {
            ...
            decltype(test_std::connect(
                std::declval<Sender>(),
                std::declval<upstream<Result, Receiver>>()
            )) inner_state;
            template <typename R>
            state(auto&& sender, auto&& expect, R&& receiver)
                : expect(expect)
                , receiver(std::forward<R>(receiver))
                , inner_state(test_std::connect(
                    std::move(sender),
            ...
            {
                test_std::start(this->inner_state);
            }
        };
        ...
        template <typename S>
        test_sender(S&& c, std::tuple<Tag, T...> const& value)
            : sender(std::forward<S>(c))
        {
    ...
    template <typename> struct add_value_sig;
    template <typename... S>
    ...
    {
        using sender_concept = test_std::sender_t;
        auto get_completion_signatures(auto const& env) const noexcept {
    ...
    auto test_when_all() -> void
    {
        ...
        test_when_all_results(std::tuple{test_std::set_value, 'a', true, 3.5},
                              test_std::just(), test_std::just('a'), test_std::just(true, 3.5));
        test_when_all_results(std::tuple{test_std::set_value, 17}, test_std::just() | test_std::then([]{ return 17; }));
        test_when_all_results(std::tuple{test_std::set_value, 17}, test_std::just() | test_std::then([]()noexcept{ return 17; }));
        ...
        test_when_all_results(std::tuple{test_std::set_error, 17},
                              test_std::just(), add_value{test_std::just_error(17)}, test_std::just(true, 3.5));
        test_when_all_results(std::tuple{test_std::set_error, 17},
                              await_cancel(), add_value{test_std::just_error(17)}, test_std::just(true, 3.5));
        test_when_all_results(std::tuple{test_std::set_stopped},
                              test_std::just(), add_value{test_std::just_stopped()}, test_std::just(true, 3.5));
        test_when_all_results(std::tuple{test_std::set_stopped},
