// src/beman/execution26/tests/exec-into-variant.pass.cpp             -*-C++-*-
...
namespace
{
    ...
    auto test_into_variant(auto&& sender) -> void
    {
        ...
        if constexpr (Expect)
        {
            ...
            static_assert(std::same_as<
                Signatures,
                decltype(test_std::get_completion_signatures(test_std::into_variant(sender), test_std::empty_env{}))
            >);
            bool called{false};
    ...
    struct error_receiver
    {
        ...
        auto set_error(int e) && noexcept -> void
        {
            *this->called = e == 17;
        }
...
auto main() -> int
{
    ...
    static_assert(std::same_as<
        test_std::into_variant_t const,
        decltype(test_std::into_variant)
    >);
    ...
    test_into_variant<true,
        ...
        >(sender_in<test_std::completion_signatures<>>{});

    test_into_variant<true,
        test_std::completion_signatures<
            test_std::set_value_t(std::variant<std::tuple<>>)
            >
        >(sender_in<test_std::completion_signatures<
    ...
    test_into_variant<true,
        test_std::completion_signatures<
            test_std::set_value_t(std::variant<
                std::tuple<>,
                std::tuple<arg, arg>
                >),
            test_std::set_error_t(error),
