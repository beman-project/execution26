...
template <typename Tag>
struct get_completion_scheduler_t : ::beman::execution26::forwarding_query_t {
    ...
    template <typename Env>
        requires(
                    ...
                    (not requires(const get_completion_scheduler_t&                                    self,
                                  const get_completion_scheduler_t<::beman::execution26::set_value_t>& value_self,
                                  const ::std::remove_cvref_t<Env>&                                    env) {
                        { env.query(self) } noexcept -> ::beman::execution26::detail::almost_scheduler;
                        {
                            ::beman::execution26::get_env(::beman::execution26::schedule(env.query(self)))
                                .query(value_self)
    ...
    template <typename Env>
        requires requires(const get_completion_scheduler_t&                                    self,
            ...
            { env.query(self) } noexcept -> ::beman::execution26::detail::almost_scheduler;
            {
                ::beman::execution26::get_env(::beman::execution26::schedule(env.query(self))).query(value_self)
            } -> ::beman::execution26::detail::decayed_same_as<decltype(env.query(self))>;
        }
