...
struct get_domain_late_helper {
    ...
    static constexpr auto get(const Sender& sender, const Env& env) noexcept {
        if constexpr (requires {
                          ...
                          } -> ::beman::execution26::detail::not_void;
                      })
            return ::beman::execution26::get_domain(::beman::execution26::get_env(sender));
        else if constexpr (requires {
                               {
                                   ::beman::execution26::detail::completion_domain<void>(sender)
                               } -> ::beman::execution26::detail::not_void;
                           })
            return ::beman::execution26::detail::completion_domain<void>(sender);
        else if constexpr (requires {
                               { ::beman::execution26::get_domain(env) } -> ::beman::execution26::detail::not_void;
                           })
            return ::beman::execution26::get_domain(env);
        else if constexpr (requires {
                               {
                                   ::beman::execution26::get_domain(::beman::execution26::get_scheduler(env))
                               } -> ::beman::execution26::detail::not_void;
                           })
